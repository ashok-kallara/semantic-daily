...
SYSTEM_MSG_EVAL = """You are an expert AI news judge scoring articles across three dimensions:
...
3. Category (string): A short, punchy category name (e.g. "🚀 Product Launches", "🧠 Research", "💼 Meta/Business", "🛠️ Tools") that best fits the article. Use an emoji.

Output exactly a JSON list of objects. DO NOT INCLUDE NOISE OR MARKDOWN. Example:
[{"index": 0, "relevance": 8, "wit": 2, "category": "🧠 Research"}, {"index": 1, "relevance": 3, "wit": 9, "category": "💬 Social Drama"}]"""
...
class PersonaProcessor:
    ...
    async def evaluate_articles(self, articles: list[Article]) -> list[Article]:
        ...
        for i in range(0, len(articles), batch_size):
            ...
            try:
                response = await self.llm_evaluator.generate(SYSTEM_MSG_EVAL, prompt, max_tokens=2500)
                if not response:
                ...
                for score_data in scores:
                    ...
                    cat = score_data.get("category", "📡 Updates")
